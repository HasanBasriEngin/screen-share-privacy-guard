...
(function (root) {
  ...
  // İsim tespiti Faz A: (1) "Ad Soyad:", "Alıcı:" gibi Türkçe etiketlerin hemen
  // ardından gelen büyük harfli 2-3 kelimelik diziyi isim say (yüksek isabet,
  // sadece isim kısmı bulanıklaşsın diye etiket lookbehind içinde tutulur);
  // (2) yaygın bir Türkçe ilk isimden hemen sonra gelen büyük harfli kelimeyi
  // muhtemel soyadı say (daha geniş, bazı yanlış pozitifler kabul edilebilir —
  // bkz. "Bilinçli tasarım kararları").
  function buildNamePatterns() {
    const patterns = [
      {
        name: 'isim-baglam',
        regex: /(?<=(?:^|[^A-Za-zÇĞİÖŞÜçğıöşü0-9])(?:Ad\s*Soyad|Adı\s*Soyadı|İsim|Alıcı|Müşteri|Gönderen|Teslim\s*Alan|Sipariş\s*Sahibi|Fatura\s*Adı|Kart\s*Sahibi)\s*:?\s*)[A-ZÇĞİÖŞÜ][a-zçğıöşü]+(?:\s+[A-ZÇĞİÖŞÜ][a-zçğıöşü]+){1,2}/g,
        validate: () => true,
        label: 'İsim'
      }
    ];
